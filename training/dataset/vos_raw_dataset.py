...
class CityscapesDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        video_mask_path = os.path.join(self.gt_folder, video_name.replace("leftImg8bit", "gtFine_labelTrainIds") + ".png")

        segment_loader = PNGSegmentLoader(video_mask_path)
        ...
        video_name = video_name.split("_")[-1]  # filename is sa_{int}
        # video id needs to be image_id to be able to load correct annotation file during eval
        video = VOSVideo(video_name, int(idx), frames)
        return video, segment_loader
